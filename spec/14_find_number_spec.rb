...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        average = (game_guessing.min + game_guessing.max) / 2
  ...
  describe '#game_over?' do
    let(:random) { double('random_number', value: 4) }
    context 'when guess and random_number are equal' do
      ...
      subject(:game_end) { described_class.new(0, 9, random, 4) }
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_end) { described_class.new(0, 9, random, 6) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        update = low_guess_game.guess + 1
        expect(low_guess_game.min).to eq(update)
      end

      it 'does not update max' do
        maximum = low_guess_game.max
        expect(maximum).to eq(maximum)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        ...
        expect(minimum).to eq(minimum)
      end

      it 'updates max to 8' do
        update = high_guess_game.guess - 1
        expect(high_guess_game.max).to eq(update)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:game_edge) { described_class.new(5, 8, number_range, 7) }

      before do
        game_edge.update_range
      end

      it 'updates min to the same value as max' do
        update = game_edge.guess + 1
        expect(game_edge.min).to eq(update)
      end

      it 'does not update max' do
        maximum = game_edge.max
        expect(maximum).to eq(maximum)
      end
    end
  end
